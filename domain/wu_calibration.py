"""Calibración pura de sensores para estaciones Weather Underground."""
...
def has_effective_wu_calibration(raw: Mapping[str, Any] | None) -> bool:
    """True si hay al menos un offset distinto de cero tras normalizar."""
    if not isinstance(raw, Mapping):
        return False
    normalized = normalize_wu_calibration(raw)
    return any(abs(float(value)) > 1e-9 for value in normalized.values())


def looks_like_min_bound_calibration_artifact(raw: Mapping[str, Any] | None) -> bool:
    """
    Detecta el bug de UI en el que Streamlit inicializaba cada number_input
    con su ``min_value`` cuando no existía calibración real guardada.

    La firma es muy específica: varios sensores exactamente en su límite
    mínimo y ningún sensor con otro offset distinto de cero.
    """
    if not isinstance(raw, Mapping):
        return False
    min_bound_hits = 0
    ...
    for key, spec in WU_CALIBRATION_SPECS.items():
        ...
        if _is_nan(value) or abs(float(value)) <= 1e-9:
            continue
        if abs(float(value) - float(spec["min"])) <= 1e-9:
            min_bound_hits += 1
        else:
            other_offsets += 1
    return min_bound_hits >= 2 and other_offsets == 0
...
def apply_wu_series_calibration(series: Mapping[str, Any] | None, calibration: Mapping[str, Any] | None) -> Dict[str, Any]:
    ...
    raw_extremes = payload.get("daily_extremes")
    if isinstance(raw_extremes, Mapping):
        calibrated_extremes = dict(raw_extremes)
        for key in ("temp_max", "temp_min"):
            _apply_scalar(calibrated_extremes, key, cal["thermometer"])
        for key in ("rh_max", "rh_min"):
            _apply_scalar(calibrated_extremes, key, cal["hygrometer"], minimum=0.0, maximum=100.0)
        _apply_scalar(calibrated_extremes, "gust_max", cal["anemometer"], minimum=0.0)
        payload["daily_extremes"] = calibrated_extremes
