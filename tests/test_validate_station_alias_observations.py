...
def test_hourly_source_uses_the_observation_nearest_the_hour():
    base = 1767225600
    series = {
        "epochs": [base, base + 10 * 60, base + 50 * 60],
        "temps": [10.0, 11.0, 15.0],
        "winds": [2.0, 3.0, 8.0],
    }
    hourly = validator._hourly_source(series)
    assert hourly[base // 3600]["epoch"] == base
    assert hourly[base // 3600]["temperature_c"] == 10.0
    assert hourly[base // 3600]["wind_kmh"] == 2.0


def test_compare_hourly_confirms_repeated_matching_measurements():
    source = {hour: {"temperature_c": 20.0, "humidity_pct": 60.0} for hour in range(12)}
    iem = {hour: {"temperature_c": 20.1, "humidity_pct": 60.5} for hour in range(12)}
    result = compare_hourly(source, iem)
    assert result["status"] == "confirmed"
    assert result["matched_hours"] == 12


def test_compare_hourly_rejects_repeated_temperature_conflict():
    source = {hour: {"temperature_c": 20.0, "humidity_pct": 60.0} for hour in range(12)}
    iem = {hour: {"temperature_c": 25.0, "humidity_pct": 80.0} for hour in range(12)}
    result = compare_hourly(source, iem)
    assert result["status"] == "conflict"


def test_compare_hourly_needs_twelve_hours():
    source = {hour: {"temperature_c": 20.0} for hour in range(11)}
    iem = {hour: {"temperature_c": 20.0} for hour in range(11)}
...
async def test_validate_batch_processes_candidates_concurrently(tmp_path, monkeypatch):
    ...
    connection.executescript(
        ...
        INSERT INTO stations VALUES
            ...
            (4, 'AEMET', 'SRC2', NULL, 'Source Station 2', 'UTC');
    ...
    async def fake_source_series(provider, station_id, client):
        base = 1767225600
        return {
                "epochs": [base + hour * 3600 for hour in range(12)],
                "temps": [20.0] * 12,
                "humidities": [60.0] * 12,
        }
    ...
    async def fake_iem_rows(network, station_id, dates, client):
        return [
            {"utc_valid": f"2026-01-01T{hour:02d}:00:00+00:00", "tmpf": 68.0, "relh": 60.0}
            for hour in range(12)
        ]
    ...
    counts = await validate_batch(
        database, limit=2, provider="AEMET", concurrency=2, include_secure=True,
    )
