"""
Servicio para integrar WeatherLink v2 como conexión personal.
"""

from __future__ import annotations

import hashlib
import logging
import math
import os
from datetime import datetime, timedelta, timezone
from typing import Any, Dict, Iterable, List, Optional, Tuple
from zoneinfo import ZoneInfo, ZoneInfoNotFoundError

import requests
import streamlit as st

logger = logging.getLogger(__name__)

from utils.provider_state import (
    clear_provider_runtime_error,
    get_connected_provider_station_id,
    is_provider_connection,
    resolve_state,
    set_provider_runtime_error,
)


WEATHERLINK_BASE_URL = os.getenv("WEATHERLINK_BASE_URL", "https://api.weatherlink.com/v2").rstrip("/")
WEATHERLINK_TIMEOUT_SECONDS = int(os.getenv("WEATHERLINK_TIMEOUT_SECONDS", "16"))
INHG_TO_HPA = 33.8638866667
MPH_TO_KMH = 1.609344


class WeatherLinkError(Exception):
    def __init__(self, kind: str, status_code: Optional[int] = None, message: str = ""):
        self.kind = kind
        self.status_code = status_code
        self.message = message
        super().__init__(message or kind)


def _safe_float(value: Any, default: float = float("nan")) -> float:
    if value is None or isinstance(value, bool):
        return default
    try:
        return float(value)
    except (TypeError, ValueError):
        return default


def _is_nan(value: float) -> bool:
    return value != value


def _first_valid(*values: Any) -> float:
    for value in values:
        parsed = _safe_float(value)
        if not _is_nan(parsed):
            return parsed
    return float("nan")


def _f_to_c(value: Any) -> float:
    parsed = _safe_float(value)
    return (parsed - 32.0) * 5.0 / 9.0 if not _is_nan(parsed) else float("nan")


def _mph_to_kmh(value: Any) -> float:
    parsed = _safe_float(value)
    return parsed * MPH_TO_KMH if not _is_nan(parsed) else float("nan")


def _inch_to_mm(value: Any) -> float:
    parsed = _safe_float(value)
    return parsed * 25.4 if not _is_nan(parsed) else float("nan")


def _inhg_to_hpa(value: Any) -> float:
    parsed = _safe_float(value)
    return parsed * INHG_TO_HPA if not _is_nan(parsed) else float("nan")


def _key_hash(value: str) -> str:
    return hashlib.sha1(str(value or "").strip().encode("utf-8")).hexdigest()[:12]


def _credential_hash(api_key: str, api_secret: str) -> str:
    return _key_hash(f"{str(api_key or '').strip()}:{str(api_secret or '').strip()}")


def _request_json(path: str, *, api_key: str, api_secret: str, params: Optional[Dict[str, Any]] = None) -> Any:
    key = str(api_key or "").strip()
    secret = str(api_secret or "").strip()
    if not key or not secret:
        raise WeatherLinkError("missing_credentials", message="Falta API Key o API Secret de WeatherLink.")

    query = {"api-key": key}
    if isinstance(params, dict):
        query.update(params)

    try:
        response = requests.get(
            f"{WEATHERLINK_BASE_URL}/{path.lstrip('/')}",
            params=query,
            headers={
                "Accept": "application/json",
                "User-Agent": "MeteoLabX/1.0 (+https://meteolabx.com)",
                "X-Api-Secret": secret,
            },
            timeout=WEATHERLINK_TIMEOUT_SECONDS,
        )
    except requests.Timeout as exc:
        raise WeatherLinkError("timeout", message=str(exc)) from exc
    except requests.RequestException as exc:
        raise WeatherLinkError("network", message=str(exc)) from exc

    if response.status_code >= 400:
        snippet = str(response.text or "").strip().replace("\n", " ")
        if len(snippet) > 220:
            snippet = snippet[:220] + "..."
        if response.status_code in (401, 403):
            kind = "unauthorized"
        elif response.status_code == 404:
            kind = "notfound"
        elif response.status_code == 429:
            kind = "ratelimit"
        else:
            kind = "http"
        raise WeatherLinkError(kind, status_code=response.status_code, message=snippet)

    try:
        return response.json()
    except ValueError as exc:
        raise WeatherLinkError("invalid_json", message=str(exc)) from exc


def _station_id(station: Dict[str, Any]) -> str:
    return str(station.get("station_id") or station.get("station_id_uuid") or "").strip()


def _station_name(station: Dict[str, Any], fallback: str = "") -> str:
    """
    Devuelve el nombre más amistoso disponible para una estación
    WeatherLink. Distintas estaciones exponen el "alias" del owner en
    campos distintos según el firmware/registro:

    - ``station_name``: el campo principal del schema v2.
    - ``name``: alias plano legacy.
    - ``username``: muchas estaciones particulares lo usan como nombre
      público (ej. ``meteo_roses``, ``pws_madrid``…).
    - ``device_name``: alias del datalogger en algunos despliegues.
    - ``gateway_id_hex``: último recurso identificable antes del id.

    Solo caemos al ``fallback`` (típicamente el ``station_id`` numérico)
    cuando ninguno de los anteriores aporta un nombre legible.
    """
    for key in ("station_name", "name", "username", "device_name", "gateway_id_hex"):
        value = station.get(key)
        if value is None:
            continue
        text = str(value).strip()
        if text:
            return text
    # Log temporal para diagnóstico: si caemos al fallback, dejamos en
    # logs los nombres de campos disponibles (sin valores sensibles) para
    # poder identificar qué campo expone WeatherLink el alias amistoso
    # en esa estación concreta. Eliminar este log una vez confirmado el
    # mapeo correcto en producción.
    try:
        keys_preview = sorted(k for k in station.keys() if isinstance(k, str))
        logger.info(
            "WeatherLink station sin nombre amistoso; usando id=%r. "
            "Campos disponibles: %s",
            fallback,
            keys_preview,
        )
    except Exception:
        pass
    return fallback


def normalize_weatherlink_stations(payload: Any) -> List[Dict[str, Any]]:
    stations = payload.get("stations", []) if isinstance(payload, dict) else []
    out: List[Dict[str, Any]] = []
    for item in stations if isinstance(stations, list) else []:
        if not isinstance(item, dict):
            continue
        sid = _station_id(item)
        if not sid:
            continue
        station = dict(item)
        station["station_id"] = sid
        station["station_name"] = _station_name(station, sid)
        out.append(station)
    return out


@st.cache_data(ttl=900, show_spinner=False)
def _fetch_weatherlink_stations_payload(api_key: str, credential_hash: str, _api_secret: str) -> Any:
    _ = credential_hash
    return _request_json("stations", api_key=api_key, api_secret=_api_secret)


def fetch_weatherlink_stations(api_key: str, api_secret: str) -> Dict[str, Any]:
    try:
        payload = _fetch_weatherlink_stations_payload(api_key, _credential_hash(api_key, api_secret), api_secret)
    except WeatherLinkError as exc:
        return {"ok": False, "error": str(exc), "kind": exc.kind, "stations": []}
    stations = normalize_weatherlink_stations(payload)
    return {"ok": True, "error": "", "stations": stations, "raw": payload}


@st.cache_data(ttl=30, show_spinner=False)
def _fetch_weatherlink_current_payload(
    station_id: str,
    api_key: str,
    credential_hash: str,
    _api_secret: str,
) -> Any:
    _ = credential_hash
    return _request_json(f"current/{station_id}", api_key=api_key, api_secret=_api_secret)


def fetch_weatherlink_current(station_id: str, api_key: str, api_secret: str, credential_hash: str = "") -> Dict[str, Any]:
    sid = str(station_id or "").strip()
    if not sid:
        return {"ok": False, "error": "Falta station_id de WeatherLink.", "payload": {}}
    try:
        payload = _fetch_weatherlink_current_payload(
            sid,
            api_key,
            credential_hash or _credential_hash(api_key, api_secret),
            api_secret,
        )
    except WeatherLinkError as exc:
        return {"ok": False, "error": str(exc), "kind": exc.kind, "payload": {}}
    return {"ok": True, "error": "", "payload": payload}


@st.cache_data(ttl=300, show_spinner=False)
def _fetch_weatherlink_historic_payload(
    station_id: str,
    api_key: str,
    credential_hash: str,
    _api_secret: str,
    start_ts: int,
    end_ts: int,
) -> Any:
    _ = credential_hash
    return _request_json(
        f"historic/{station_id}",
        api_key=api_key,
        api_secret=_api_secret,
        params={
            "start-timestamp": int(start_ts),
            "end-timestamp": int(end_ts),
        },
    )


def fetch_weatherlink_historic(
    station_id: str,
    api_key: str,
    api_secret: str,
    *,
    start_ts: int,
    end_ts: int,
    credential_hash: str = "",
) -> Dict[str, Any]:
    sid = str(station_id or "").strip()
    if not sid:
        return {"ok": False, "error": "Falta station_id de WeatherLink.", "payload": {}}
    if int(end_ts or 0) <= int(start_ts or 0):
        return {"ok": False, "error": "Rango histórico WeatherLink inválido.", "payload": {}}
    try:
        payload = _fetch_weatherlink_historic_payload(
            sid,
            api_key,
            credential_hash or _credential_hash(api_key, api_secret),
            api_secret,
            int(start_ts),
            int(end_ts),
        )
    except WeatherLinkError as exc:
        return {"ok": False, "error": str(exc), "kind": exc.kind, "payload": {}}
    return {"ok": True, "error": "", "payload": payload}


def find_weatherlink_station(stations: Iterable[Dict[str, Any]], station_id: str) -> Dict[str, Any]:
    target = str(station_id or "").strip()
    if not target:
        return {}
    for station in stations or []:
        if not isinstance(station, dict):
            continue
        if _station_id(station) == target or str(station.get("station_id_uuid", "")).strip() == target:
            return dict(station)
    return {}


def _current_records(payload: Dict[str, Any]) -> List[Tuple[int, int, int, Dict[str, Any]]]:
    records: List[Tuple[int, int, int, Dict[str, Any]]] = []
    sensors = payload.get("sensors", []) if isinstance(payload, dict) else []
    for sensor in sensors if isinstance(sensors, list) else []:
        if not isinstance(sensor, dict):
            continue
        sensor_type = int(_safe_float(sensor.get("sensor_type"), -1))
        structure = int(_safe_float(sensor.get("data_structure_type"), -1))
        for row in sensor.get("data", []) if isinstance(sensor.get("data"), list) else []:
            if not isinstance(row, dict):
                continue
            epoch = int(_safe_float(row.get("ts"), 0) or 0)
            if epoch <= 0:
                continue
            records.append((epoch, sensor_type, structure, row))
    records.sort(key=lambda item: (item[0], _record_priority(item[1], item[2])), reverse=True)
    return records


def _record_priority(sensor_type: int, structure: int) -> int:
    if structure in (6, 10, 23):
        return 100
    if structure in (3, 4, 7, 11, 24):
        return 95
    if structure in (12, 13, 20):
        return 85
    if sensor_type == 3 or structure == 9:
        return 80
    return 10


def _first_from_records(records: List[Tuple[int, int, int, Dict[str, Any]]], keys: Iterable[str], *, convert=None) -> float:
    for _epoch, _sensor_type, _structure, row in records:
        value = _first_valid(*(row.get(key) for key in keys))
        if not _is_nan(value):
            return convert(value) if callable(convert) else value
    return float("nan")


def _values_from_records(records: List[Tuple[int, int, int, Dict[str, Any]]], keys: Iterable[str], *, convert=None) -> List[float]:
    values: List[float] = []
    for _epoch, _sensor_type, _structure, row in records:
        value = _first_valid(*(row.get(key) for key in keys))
        if not _is_nan(value):
            values.append(convert(value) if callable(convert) else value)
    return values


def _with_fallback(value: float, fallback: float) -> float:
    return fallback if _is_nan(value) else value


def _series_date_key(epoch: int) -> str:
    try:
        return datetime.fromtimestamp(int(epoch)).date().isoformat()
    except Exception:
        return ""


def _valid_values(values: Iterable[Any]) -> List[float]:
    valid: List[float] = []
    for value in values or []:
        parsed = _safe_float(value)
        if not _is_nan(parsed):
            valid.append(parsed)
    return valid


def _append_current_to_local_series(data: Dict[str, Any], state: Any) -> Dict[str, Any]:
    """
    WeatherLink ``/current`` no entrega una serie intradía. Guardamos las
    lecturas que ya hemos recibido en la sesión para que las gráficas y los
    extremos no se queden vacíos mientras la app sigue abierta.
    """
    if state is None:
        return data

    raw_series = data.get("_series")
    if not isinstance(raw_series, dict) or not raw_series.get("epochs"):
        return data

    station_id = str(data.get("station_code") or data.get("idema") or "").strip()
    epoch = int(_safe_float(raw_series.get("epochs", [0])[-1], 0) or 0)
    if epoch <= 0:
        return data

    date_key = _series_date_key(epoch)
    cache_key = "_weatherlink_local_day_series"
    cache_station_key = "_weatherlink_local_day_series_station"
    cache_date_key = "_weatherlink_local_day_series_date"
    series_keys = (
        "epochs", "temps", "humidities", "dewpts",
        "pressures_abs", "pressures_msl", "winds", "gusts",
        "wind_dirs", "precips", "solar_radiations", "uv_indexes",
    )

    cached = state.get(cache_key)
    if (
        not isinstance(cached, dict)
        or str(state.get(cache_station_key) or "") != station_id
        or str(state.get(cache_date_key) or "") != date_key
    ):
        cached = {key: [] for key in series_keys}

    for key in series_keys:
        cached.setdefault(key, [])

    incoming_index = len(raw_series.get("epochs", [])) - 1
    if cached["epochs"] and int(_safe_float(cached["epochs"][-1], 0) or 0) == epoch:
        target_index = len(cached["epochs"]) - 1
    else:
        cached["epochs"].append(epoch)
        target_index = len(cached["epochs"]) - 1
        for key in series_keys:
            if key != "epochs":
                cached[key].append(float("nan"))

    for key in series_keys:
        if key == "epochs":
            continue
        values = raw_series.get(key, [])
        value = values[incoming_index] if incoming_index < len(values) else float("nan")
        cached[key][target_index] = value

    max_points = 24 * 12
    if len(cached["epochs"]) > max_points:
        for key in series_keys:
            cached[key] = list(cached.get(key, []))[-max_points:]

    cached["has_data"] = bool(cached.get("epochs"))
    state[cache_key] = cached
    state[cache_station_key] = station_id
    state[cache_date_key] = date_key

    merged = {key: list(cached.get(key, [])) for key in series_keys}
    merged["has_data"] = True
    data["_series"] = merged

    temp_values = _valid_values(merged.get("temps", []))
    rh_values = _valid_values(merged.get("humidities", []))
    gust_values = _valid_values(merged.get("gusts", []))
    if temp_values:
        if _is_nan(_safe_float(data.get("temp_max"))):
            data["temp_max"] = max(temp_values)
        if _is_nan(_safe_float(data.get("temp_min"))):
            data["temp_min"] = min(temp_values)
    if rh_values:
        if _is_nan(_safe_float(data.get("rh_max"))):
            data["rh_max"] = max(rh_values)
        if _is_nan(_safe_float(data.get("rh_min"))):
            data["rh_min"] = min(rh_values)
    if gust_values and _is_nan(_safe_float(data.get("gust_max"))):
        data["gust_max"] = max(gust_values)

    return data


def _station_tzinfo(station: Dict[str, Any]):
    tz_name = str(station.get("time_zone") or "").strip()
    if tz_name:
        try:
            return ZoneInfo(tz_name)
        except (ZoneInfoNotFoundError, ValueError):
            pass
    return datetime.now().astimezone().tzinfo or timezone.utc


def _today_window(station: Dict[str, Any], *, now_epoch: Optional[int] = None) -> Tuple[int, int]:
    tzinfo = _station_tzinfo(station)
    if now_epoch is None:
        now_dt = datetime.now(tzinfo)
    else:
        now_dt = datetime.fromtimestamp(int(now_epoch), tzinfo)
    day_start = now_dt.replace(hour=0, minute=0, second=0, microsecond=0)
    end_dt = now_dt.replace(second=0, microsecond=0)
    end_dt = end_dt - timedelta(minutes=end_dt.minute % 5)
    if end_dt <= day_start:
        end_dt = now_dt.replace(second=0, microsecond=0)
    return int(day_start.timestamp()), int(end_dt.timestamp())


def _first_precip_mm(rows: List[Tuple[int, int, int, Dict[str, Any]]]) -> float:
    value = _first_from_records(rows, ("rainfall_mm", "rainfall_daily_mm", "rain_rate_last_mm"))
    if not _is_nan(value):
        return value
    return _first_from_records(rows, ("rainfall_in", "rainfall_daily_in"), convert=_inch_to_mm)


def _series_value_from_rows(
    rows: List[Tuple[int, int, int, Dict[str, Any]]],
    keys: Iterable[str],
    *,
    convert=None,
) -> float:
    return _first_from_records(rows, keys, convert=convert)


def normalize_weatherlink_historic_series(payload: Dict[str, Any], altitude_m: Any = None) -> Dict[str, Any]:
    records = _current_records(payload if isinstance(payload, dict) else {})
    elevation = _first_valid(altitude_m, 0.0)
    if _is_nan(elevation):
        elevation = 0.0

    grouped: Dict[int, List[Tuple[int, int, int, Dict[str, Any]]]] = {}
    for epoch, sensor_type, structure, row in records:
        grouped.setdefault(int(epoch), []).append((int(epoch), sensor_type, structure, row))

    series = {
        "epochs": [],
        "temps": [],
        "humidities": [],
        "dewpts": [],
        "pressures_abs": [],
        "pressures_msl": [],
        "winds": [],
        "gusts": [],
        "wind_dirs": [],
        "precips": [],
        "solar_radiations": [],
        "uv_indexes": [],
        "has_data": False,
    }

    for epoch in sorted(grouped):
        rows = sorted(
            grouped[epoch],
            key=lambda item: _record_priority(item[1], item[2]),
            reverse=True,
        )
        temp_c = _series_value_from_rows(rows, ("temp_last", "temp", "temp_out"), convert=_f_to_c)
        rh = _series_value_from_rows(rows, ("hum_last", "hum", "hum_out"))
        dew_c = _series_value_from_rows(rows, ("dew_point_last", "dew_point", "dew_point_out"), convert=_f_to_c)
        wind_kmh = _series_value_from_rows(
            rows,
            ("wind_speed_avg", "wind_speed_last", "wind_speed_avg_last_10_min"),
            convert=_mph_to_kmh,
        )
        gust_kmh = _series_value_from_rows(
            rows,
            ("wind_speed_hi", "wind_speed_hi_last_10_min", "wind_gust"),
            convert=_mph_to_kmh,
        )
        wind_dir = _series_value_from_rows(
            rows,
            ("wind_dir_of_prevail", "wind_dir_scalar_avg", "wind_dir_last"),
        )
        p_msl = _series_value_from_rows(rows, ("bar", "bar_sea_level", "pressure_last", "bar_alt"), convert=_inhg_to_hpa)
        p_abs = _series_value_from_rows(rows, ("bar_absolute", "abs_press"), convert=_inhg_to_hpa)
        if _is_nan(p_abs):
            p_abs = _pressure_abs_from_msl(p_msl, float(elevation))
        precip_mm = _first_precip_mm(rows)
        solar = _series_value_from_rows(rows, ("solar_rad_avg", "solar_rad", "solar_rad_hi"))
        uv = _series_value_from_rows(rows, ("uv_index_avg", "uv_index", "uv_index_hi"))

        if all(_is_nan(value) for value in (temp_c, rh, dew_c, p_abs, p_msl, wind_kmh, gust_kmh, precip_mm, solar, uv)):
            continue

        series["epochs"].append(int(epoch))
        series["temps"].append(float(temp_c))
        series["humidities"].append(float(rh))
        series["dewpts"].append(float(dew_c))
        series["pressures_abs"].append(float(p_abs))
        series["pressures_msl"].append(float(p_msl))
        series["winds"].append(float(wind_kmh))
        series["gusts"].append(float(gust_kmh))
        series["wind_dirs"].append(float(wind_dir))
        series["precips"].append(float(precip_mm))
        series["solar_radiations"].append(float(solar))
        series["uv_indexes"].append(float(uv))

    temp_highs = _values_from_records(records, ("temp_hi", "temp_high", "temp_out_hi"), convert=_f_to_c)
    temp_lows = _values_from_records(records, ("temp_low", "temp_lo", "temp_min", "temp_out_low"), convert=_f_to_c)
    rh_highs = _values_from_records(records, ("hum_hi", "hum_high", "hum_out_hi"))
    rh_lows = _values_from_records(records, ("hum_low", "hum_lo", "hum_min", "hum_out_low"))
    gust_highs = _values_from_records(records, ("wind_speed_hi", "wind_speed_hi_day", "wind_gust_hi"), convert=_mph_to_kmh)

    temp_values = _valid_values(series["temps"])
    rh_values = _valid_values(series["humidities"])
    gust_values = _valid_values(series["gusts"])
    series["_extremes"] = {
        "temp_max": max(temp_highs or temp_values, default=float("nan")),
        "temp_min": min(temp_lows or temp_values, default=float("nan")),
        "rh_max": max(rh_highs or rh_values, default=float("nan")),
        "rh_min": min(rh_lows or rh_values, default=float("nan")),
        "gust_max": max(gust_highs or gust_values, default=float("nan")),
    }
    series["has_data"] = bool(series["epochs"])
    return series


def _merge_weatherlink_series(historic: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(historic, dict) or not historic.get("has_data"):
        return current if isinstance(current, dict) else {}
    if not isinstance(current, dict) or not current.get("epochs"):
        return historic

    series_keys = (
        "epochs", "temps", "humidities", "dewpts",
        "pressures_abs", "pressures_msl", "winds", "gusts",
        "wind_dirs", "precips", "solar_radiations", "uv_indexes",
    )
    merged = {key: list(historic.get(key, [])) for key in series_keys}
    current_epoch = int(_safe_float(current.get("epochs", [0])[-1], 0) or 0)
    if current_epoch <= 0:
        merged["has_data"] = True
        if "_extremes" in historic:
            merged["_extremes"] = historic["_extremes"]
        return merged

    if merged["epochs"] and current_epoch <= int(_safe_float(merged["epochs"][-1], 0) or 0):
        target_index = len(merged["epochs"]) - 1
    else:
        merged["epochs"].append(current_epoch)
        target_index = len(merged["epochs"]) - 1
        for key in series_keys:
            if key != "epochs":
                merged.setdefault(key, []).append(float("nan"))

    current_index = len(current.get("epochs", [])) - 1
    for key in series_keys:
        if key == "epochs":
            continue
        values = current.get(key, [])
        if current_index < len(values):
            merged[key][target_index] = values[current_index]

    merged["has_data"] = True
    if "_extremes" in historic:
        merged["_extremes"] = historic["_extremes"]
    return merged


def _empty_weatherlink_series() -> Dict[str, Any]:
    return {
        "epochs": [],
        "temps": [],
        "humidities": [],
        "dewpts": [],
        "pressures_abs": [],
        "pressures_msl": [],
        "winds": [],
        "gusts": [],
        "wind_dirs": [],
        "precips": [],
        "solar_radiations": [],
        "uv_indexes": [],
        "has_data": False,
    }


def _series_keys() -> Tuple[str, ...]:
    return (
        "epochs", "temps", "humidities", "dewpts",
        "pressures_abs", "pressures_msl", "winds", "gusts",
        "wind_dirs", "precips", "solar_radiations", "uv_indexes",
    )


def _sort_dedupe_series(series: Dict[str, Any]) -> Dict[str, Any]:
    epochs = list(series.get("epochs", []) or [])
    rows: List[Tuple[int, int]] = []
    for index, epoch in enumerate(epochs):
        epoch_int = int(_safe_float(epoch, 0) or 0)
        if epoch_int > 0:
            rows.append((epoch_int, index))
    rows.sort(key=lambda item: item[0])

    deduped = _empty_weatherlink_series()
    seen: set[int] = set()
    for epoch_int, index in rows:
        if epoch_int in seen:
            continue
        seen.add(epoch_int)
        deduped["epochs"].append(epoch_int)
        for key in _series_keys():
            if key == "epochs":
                continue
            values = list(series.get(key, []) or [])
            deduped[key].append(values[index] if index < len(values) else float("nan"))

    deduped["has_data"] = bool(deduped["epochs"])
    return deduped


def fetch_weatherlink_historic_series(
    station_id: str,
    api_key: str,
    api_secret: str,
    *,
    start_ts: int,
    end_ts: int,
    altitude_m: Any = None,
    credential_hash: str = "",
    max_chunk_seconds: int = 24 * 3600,
) -> Dict[str, Any]:
    """
    Descarga y normaliza histórico WeatherLink en trozos de 24 h.

    WeatherLink limita el tamaño de las respuestas históricas; hacer chunks
    diarios mantiene la llamada estable y aprovecha ``st.cache_data`` por
    tramo. Si la cuenta no tiene histórico (Pro/Pro+), devolvemos ``has_data``
    falso sin romper el proveedor.
    """
    sid = str(station_id or "").strip()
    start = int(start_ts or 0)
    end = int(end_ts or 0)
    if not sid or end <= start:
        return _empty_weatherlink_series()

    combined = _empty_weatherlink_series()
    extremes = {
        "temp_max": float("nan"),
        "temp_min": float("nan"),
        "rh_max": float("nan"),
        "rh_min": float("nan"),
        "gust_max": float("nan"),
    }

    cursor = start
    while cursor < end:
        chunk_end = min(end, cursor + int(max_chunk_seconds))
        payload = fetch_weatherlink_historic(
            sid,
            api_key,
            api_secret,
            start_ts=cursor,
            end_ts=chunk_end,
            credential_hash=credential_hash,
        )
        if not isinstance(payload, dict) or not payload.get("ok"):
            if not combined.get("has_data"):
                return _empty_weatherlink_series()
            break

        chunk = normalize_weatherlink_historic_series(payload.get("payload", {}), altitude_m=altitude_m)
        if chunk.get("has_data"):
            for key in _series_keys():
                combined[key].extend(list(chunk.get(key, []) or []))

            chunk_extremes = chunk.get("_extremes", {}) if isinstance(chunk.get("_extremes"), dict) else {}
            for key, value in chunk_extremes.items():
                parsed = _safe_float(value)
                if _is_nan(parsed):
                    continue
                current = _safe_float(extremes.get(key))
                if _is_nan(current):
                    extremes[key] = parsed
                elif key.endswith("_min"):
                    extremes[key] = min(current, parsed)
                else:
                    extremes[key] = max(current, parsed)

        cursor = chunk_end

    combined = _sort_dedupe_series(combined)
    combined["_extremes"] = extremes
    return combined


def _weatherlink_station_meta_from_state(state: Any, station_id: str) -> Dict[str, Any]:
    stations = state.get("weatherlink_stations", []) if state is not None else []
    station_meta = find_weatherlink_station(stations if isinstance(stations, list) else [], station_id)
    if station_meta:
        return station_meta
    return {
        "station_id": station_id,
        "station_name": state.get("weatherlink_station_name") if state is not None else station_id,
        "latitude": state.get("weatherlink_station_lat") if state is not None else None,
        "longitude": state.get("weatherlink_station_lon") if state is not None else None,
        "elevation": state.get("weatherlink_station_alt") if state is not None else None,
        "time_zone": state.get("weatherlink_station_tz") if state is not None else None,
    }


def _weatherlink_credentials_from_state(state: Any) -> Tuple[str, str, str]:
    station_id = get_connected_provider_station_id("WEATHERLINK", state)
    api_key = str(state.get("weatherlink_api_key", "") or "").strip()
    api_secret = str(state.get("weatherlink_api_secret", "") or "").strip()
    return station_id, api_key, api_secret


def fetch_weatherlink_today_series_with_lookback(
    station_id: str = "",
    *,
    hours_before_start: int = 3,
    state=None,
) -> Dict[str, Any]:
    state = resolve_state(state)
    station_id_state, api_key, api_secret = _weatherlink_credentials_from_state(state)
    sid = str(station_id or station_id_state or "").strip()
    if not sid or not api_key or not api_secret:
        return _empty_weatherlink_series()

    station_meta = _weatherlink_station_meta_from_state(state, sid)
    now_epoch = int(datetime.now(timezone.utc).timestamp())
    day_start, end_ts = _today_window(station_meta, now_epoch=now_epoch)
    start_ts = max(0, int(day_start) - max(0, int(hours_before_start or 0)) * 3600)
    altitude = state.get("weatherlink_station_alt", station_meta.get("elevation", 0.0))
    return fetch_weatherlink_historic_series(
        sid,
        api_key,
        api_secret,
        start_ts=start_ts,
        end_ts=end_ts,
        altitude_m=altitude,
        credential_hash=f"{_key_hash(api_key)}:{_key_hash(api_secret)}",
    )


def fetch_weatherlink_recent_synoptic_series(
    station_id: str = "",
    *,
    days: int = 7,
    state=None,
) -> Dict[str, Any]:
    state = resolve_state(state)
    station_id_state, api_key, api_secret = _weatherlink_credentials_from_state(state)
    sid = str(station_id or station_id_state or "").strip()
    if not sid or not api_key or not api_secret:
        return _empty_weatherlink_series()

    station_meta = _weatherlink_station_meta_from_state(state, sid)
    _day_start, end_ts = _today_window(station_meta, now_epoch=int(datetime.now(timezone.utc).timestamp()))
    start_ts = max(0, int(end_ts) - max(1, int(days or 7)) * 24 * 3600)
    altitude = state.get("weatherlink_station_alt", station_meta.get("elevation", 0.0))
    return fetch_weatherlink_historic_series(
        sid,
        api_key,
        api_secret,
        start_ts=start_ts,
        end_ts=end_ts,
        altitude_m=altitude,
        credential_hash=f"{_key_hash(api_key)}:{_key_hash(api_secret)}",
    )


def _latest_epoch(records: List[Tuple[int, int, int, Dict[str, Any]]], payload: Dict[str, Any]) -> int:
    for epoch, _sensor_type, _structure, _row in records:
        if epoch > 0:
            return int(epoch)
    generated = int(_safe_float(payload.get("generated_at"), 0) or 0)
    return generated if generated > 0 else int(datetime.now(timezone.utc).timestamp())


def _pressure_abs_from_msl(p_msl_hpa: float, elevation_m: float) -> float:
    if _is_nan(p_msl_hpa):
        return float("nan")
    return p_msl_hpa / math.exp(float(elevation_m or 0.0) / 8000.0)


def normalize_weatherlink_current(payload: Dict[str, Any], station: Optional[Dict[str, Any]] = None, altitude_m: Any = None) -> Dict[str, Any]:
    station = dict(station or {})
    records = _current_records(payload if isinstance(payload, dict) else {})
    station_id = str((payload or {}).get("station_id") or _station_id(station)).strip()
    elevation = _first_valid(altitude_m, station.get("elevation"), 0.0)
    if _is_nan(elevation):
        elevation = 0.0

    temp_c = _first_from_records(records, ("temp", "temp_out"), convert=_f_to_c)
    rh = _first_from_records(records, ("hum", "hum_out"))
    dew_c = _first_from_records(records, ("dew_point", "dew_point_out"), convert=_f_to_c)
    wind_kmh = _first_from_records(
        records,
        (
            "wind_speed_last",
            "wind_speed_avg_last_1_min",
            "wind_speed_avg_last_2_min",
            "wind_speed_avg_last_10_min",
            "wind_speed_avg",
        ),
        convert=_mph_to_kmh,
    )
    gust_kmh = _first_from_records(
        records,
        (
            "wind_speed_hi_last_10_min",
            "wind_speed_hi_last_2_min",
            "wind_speed_hi",
            "wind_gust",
        ),
        convert=_mph_to_kmh,
    )
    wind_dir = _first_from_records(
        records,
        (
            "wind_dir_last",
            "wind_dir_scalar_avg_last_1_min",
            "wind_dir_scalar_avg_last_2_min",
            "wind_dir_scalar_avg_last_10_min",
            "wind_dir_of_prevail",
        ),
    )
    p_msl = _first_from_records(
        records,
        ("bar_sea_level", "pressure_last", "bar", "bar_alt"),
        convert=_inhg_to_hpa,
    )
    p_abs = _first_from_records(records, ("bar_absolute", "abs_press"), convert=_inhg_to_hpa)
    if _is_nan(p_abs):
        p_abs = _pressure_abs_from_msl(p_msl, float(elevation))

    precip_total = _first_from_records(
        records,
        ("rainfall_daily_mm", "rainfall_last_24_hr_mm", "rainfall_mm", "rain_storm_mm"),
    )
    rain_rate = _first_from_records(records, ("rain_rate_last_mm", "rain_rate_hi_mm", "rain_rate_hi_last_15_min_mm"))
    solar = _first_from_records(records, ("solar_rad", "solar_rad_hi", "solar_rad_avg"))
    uv = _first_from_records(records, ("uv_index", "uv_index_hi", "uv_index_avg"))
    heat_index_c = _first_from_records(records, ("heat_index", "heat_index_out", "thw_index"), convert=_f_to_c)
    wind_chill_c = _first_from_records(records, ("wind_chill", "wind_chill_last"), convert=_f_to_c)
    wet_bulb_c = _first_from_records(records, ("wet_bulb",), convert=_f_to_c)
    temp_max_c = _first_from_records(
        records,
        (
            "temp_hi",
            "temp_high",
            "temp_out_hi",
            "temp_out_high",
            "temp_day_high",
            "temp_high_today",
            "temp_hi_today",
        ),
        convert=_f_to_c,
    )
    temp_min_c = _first_from_records(
        records,
        (
            "temp_low",
            "temp_lo",
            "temp_min",
            "temp_out_low",
            "temp_out_lo",
            "temp_day_low",
            "temp_low_today",
            "temp_lo_today",
        ),
        convert=_f_to_c,
    )
    rh_max = _first_from_records(
        records,
        ("hum_hi", "hum_high", "hum_out_hi", "hum_out_high", "hum_day_high", "hum_hi_today"),
    )
    rh_min = _first_from_records(
        records,
        ("hum_low", "hum_lo", "hum_min", "hum_out_low", "hum_out_lo", "hum_day_low", "hum_lo_today"),
    )
    gust_max_kmh = _first_from_records(
        records,
        (
            "wind_speed_hi_day",
            "wind_speed_hi_today",
            "wind_speed_hi",
            "wind_gust_hi",
            "wind_gust_high",
            "wind_speed_hi_last_10_min",
            "wind_speed_hi_last_2_min",
            "wind_gust",
        ),
        convert=_mph_to_kmh,
    )

    epoch = _latest_epoch(records, payload if isinstance(payload, dict) else {})
    lat = _first_valid(station.get("latitude"), station.get("lat"))
    lon = _first_valid(station.get("longitude"), station.get("lon"))
    name = _station_name(station, station_id or "WeatherLink")

    series = {
        "epochs": [int(epoch)],
        "temps": [float(temp_c)],
        "humidities": [float(rh)],
        "dewpts": [float(dew_c)],
        "pressures_abs": [float(p_abs)],
        "pressures_msl": [float(p_msl)],
        "winds": [float(wind_kmh)],
        "gusts": [float(gust_kmh)],
        "wind_dirs": [float(wind_dir)],
        "precips": [float(precip_total)],
        "solar_radiations": [float(solar)],
        "uv_indexes": [float(uv)],
        "has_data": True,
    }

    return {
        "idema": station_id,
        "station_code": station_id,
        "station_name": name,
        "station_tz": str(station.get("time_zone") or ""),
        "lat": lat,
        "lon": lon,
        "elevation": float(elevation),
        "epoch": int(epoch),
        "Tc": temp_c,
        "RH": rh,
        "Td": dew_c,
        "p_hpa": p_msl,
        "p_abs_hpa": p_abs,
        "pressure_3h_ago": float("nan"),
        "epoch_3h_ago": None,
        "wind": wind_kmh,
        "gust": gust_kmh,
        "wind_dir_deg": wind_dir,
        "precip_total": precip_total,
        "rain_rate_mm_h": rain_rate,
        "solar_radiation": solar,
        "uv": uv,
        "feels_like": float("nan"),
        "heat_index": heat_index_c,
        "wind_chill": wind_chill_c,
        "wet_bulb_api": wet_bulb_c,
        "temp_max": _with_fallback(temp_max_c, temp_c),
        "temp_min": _with_fallback(temp_min_c, temp_c),
        "rh_max": _with_fallback(rh_max, rh),
        "rh_min": _with_fallback(rh_min, rh),
        "gust_max": _with_fallback(gust_max_kmh, gust_kmh),
        "_series": series,
    }


def get_weatherlink_data(state=None) -> Optional[Dict[str, Any]]:
    state = resolve_state(state)
    if not is_provider_connection("WEATHERLINK", state):
        return None

    station_id = get_connected_provider_station_id("WEATHERLINK", state)
    api_key = str(state.get("weatherlink_api_key", "") or "").strip()
    api_secret = str(state.get("weatherlink_api_secret", "") or "").strip()
    if not station_id or not api_key or not api_secret:
        set_provider_runtime_error("WEATHERLINK", "Faltan credenciales o estación WeatherLink.", state)
        return None

    stations = state.get("weatherlink_stations", [])
    station_meta = find_weatherlink_station(stations if isinstance(stations, list) else [], station_id)
    if not station_meta:
        station_meta = {
            "station_id": station_id,
            "station_name": state.get("weatherlink_station_name") or station_id,
            "latitude": state.get("weatherlink_station_lat"),
            "longitude": state.get("weatherlink_station_lon"),
            "elevation": state.get("weatherlink_station_alt"),
            "time_zone": state.get("weatherlink_station_tz"),
        }

    payload = fetch_weatherlink_current(
        station_id,
        api_key,
        api_secret,
        credential_hash=f"{_key_hash(api_key)}:{_key_hash(api_secret)}",
    )
    if not isinstance(payload, dict) or not payload.get("ok"):
        set_provider_runtime_error("WEATHERLINK", str((payload or {}).get("error", "WeatherLink no devolvió datos.")), state)
        return None

    altitude = state.get("weatherlink_station_alt", station_meta.get("elevation", 0.0))
    data = normalize_weatherlink_current(payload.get("payload", {}), station=station_meta, altitude_m=altitude)
    current_series = data.get("_series") if isinstance(data.get("_series"), dict) else {}
    start_ts, end_ts = _today_window(station_meta, now_epoch=int(data.get("epoch") or 0) or None)
    historic_payload = fetch_weatherlink_historic(
        station_id,
        api_key,
        api_secret,
        start_ts=start_ts,
        end_ts=end_ts,
        credential_hash=f"{_key_hash(api_key)}:{_key_hash(api_secret)}",
    )
    used_historic_series = False
    if isinstance(historic_payload, dict) and historic_payload.get("ok"):
        historic_series = normalize_weatherlink_historic_series(
            historic_payload.get("payload", {}),
            altitude_m=altitude,
        )
        if historic_series.get("has_data"):
            used_historic_series = True
            data["_series"] = _merge_weatherlink_series(historic_series, current_series)
            extremes = historic_series.get("_extremes", {}) if isinstance(historic_series.get("_extremes"), dict) else {}
            for key in ("temp_max", "temp_min", "rh_max", "rh_min", "gust_max"):
                value = _safe_float(extremes.get(key))
                if not _is_nan(value):
                    existing = _safe_float(data.get(key))
                    if _is_nan(existing):
                        data[key] = value
                    elif key.endswith("_min"):
                        data[key] = min(existing, value)
                    else:
                        data[key] = max(existing, value)
    if not used_historic_series:
        # Si la cuenta/estación no tiene permiso histórico (Pro/Pro+) o la
        # llamada falla, al menos mantenemos una serie local durante la sesión.
        data = _append_current_to_local_series(data, state)
    clear_provider_runtime_error("WEATHERLINK", state)
    return data
