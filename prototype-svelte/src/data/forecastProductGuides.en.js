/**
 * The map guides, in English.
 *
 * A translation of `forecastProductGuides.js`, kept as a separate module so it
 * is only downloaded by whoever reads English: all six languages bundled
 * together would add half a megabyte for text that each visitor uses one sixth
 * of.
 *
 * Products that are missing here fall back to the Spanish guide with a notice.
 * Keys must match the product ids exactly.
 */
...
const NOAA_LI = {
  label: 'NOAA/NWS · definition and interpretation of CAPE and Lifted Index',
  url: 'https://www.weather.gov/mlb/adas_glossary'
};

const SHARPPY = {
  label: 'SHARPpy · public implementation of params.py',
  url: 'https://github.com/sharppy/SHARPpy/blob/main/sharppy/sharptab/params.py'
};

const THOMPSON_2007 = {
  label: 'Thompson, Mead and Edwards (2007) · effective layer and EBWD',
  url: 'https://doi.org/10.1175/WAF969.1'
};

const RKW_1988 = {
  label: 'Rotunno, Klemp and Weisman (1988) · shear–cold pool balance',
  url: 'https://doi.org/10.1175/1520-0469(1988)045%3C0463:ATFSLL%3E2.0.CO;2'
};

const BOLTON_1980 = {
  label: 'Bolton (1980) · computation of equivalent potential temperature',
...
const NAYLOR_2012 = {
  label: 'Naylor et al. (2012) · sensitivity of simulated updraft helicity',
  url: 'https://journals.ametsoc.org/view/journals/mwre/140/7/mwr-d-11-00209.1.xml'
};

const LINE_ORIENTATION = {
  label: 'Bluestein and Weisman (2000) · shear orientation relative to the initiating line',
  url: 'https://journals.ametsoc.org/view/journals/mwre/128/9/1520-0493_2000_128_3128_tionss_2.0.co_2.xml'
};

const CELL_MOTION = {
  label: 'NOAA/NSSL · advection and propagation of convective motion',
...
'vv-lfc': {
  ...
  reflectivity: {
    what: 'Simulated maximum column reflectivity, in dBZ: the reading a radar would give if the model hydrometeors were the real ones. Of all the levels it keeps the largest value, which is what a radar sees as it sweeps.',
    interpretation: [
      'It is the most direct map for seeing where it is raining and how: it gives no accumulated amount but the structure of the moment, and there you can tell a stratiform band from a train of cells or an organised line.',
      'As a guide: below 20 dBZ is light rain or drizzle; between 20 and 35, moderate rain; above 40 there is convection and above 50, cores with possible hail. These are the same orders of magnitude as on a real radar, but computed rather than measured.',
      'AROME partly resolves convection, so the exact position of each cell is indicative: what is reliable is the type of structure and the area, not that the core lands on one particular village.',
      'No colour below 5 dBZ. In an ordinary hour nine tenths of the domain have no echo, and painting them would bury the little that matters.'
    ],
  ...
  'mslp-theta-e-850': {
    ...
    equations: [
      ...
      { label: 'LCL temperature', latex: String.raw`T_L=\left[\frac{1}{T_d-56}+\frac{\ln(T/T_d)}{800}\right]^{-1}+56` },
